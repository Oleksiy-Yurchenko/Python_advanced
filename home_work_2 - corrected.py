...
def remove_doubles(numbers_list):
    return set(numbers_list)
#В этом случае убираются вторые, третьи и т.д. копии повторяющихся элементов, а сами элементы не убираются.

print(remove_doubles(numbers_list))

#4.2) вывести 3 наибольших числа из исходного массива

def n_largest(numbers_list, n=3):
    numbers_set = set(numbers_list)     # исправлено
    if len(numbers_set) >= n:
        return sorted(numbers_set, reverse=True)[0:n]
    return sorted(numbers_set, reverse=True)[0:len(numbers_set)]
...
def find_common_keys(dict_one, dict_two):
    return set(dict_one) & set(dict_two) # or return [key for key in dict_one if key in dict_two]
...
def three_words_row(input_string):
    words_count = 0
    array = input_string.strip().split(' ')         # исправлено
    while (words_count < 3 and len(array)):         # исправлено
        first_item = array[0]
        array = array[1:]
        try:
            int(first_item)
...
print(three_words_row(' '))
print(three_words_row('hello 1 one two three 15 world'))
print(three_words_row(''))
